...
In this paper, we present new methods for inverse reinforcement learning of RRT$^*$ cost functions. Specifically we modify Maximum Margin Planning to use RRT$^*$ as a planner and explain the implications of doing so. Furthermore, we propose a caching scheme that greatly reduces the computational cost of this approach. Our methods require no additional planner assumptions other than those inherent in RRT$^*$, making them particularly easy to implement. 

We evaluate our methods on real and simulated data from a social navigation scenario. The results demonstrate that our methods achieve better performance at lower computational cost than those that learn path planning cost functions for deterministic planners.
