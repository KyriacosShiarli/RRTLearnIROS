...
\end{equation}
where $Z_{o,g}$ is the set of all possible paths that satisfy $s_0 = o, s_{l_\zeta} = g$, and may be infinite.
...
\begin{equation}
	C(\zeta^i_{o_i,g_i}) \leq C(\zeta^*_{o_i,g_i}) \quad \forall i
\end{equation}
...
\begin{equation}
	\argmin_{\mathbf{w}} \sum_{i=0}^D C(\zeta^i_{o_i,g_i}) - C(\zeta^*_{o_i,g_i}) \label{eq:back_minim}
\end{equation}

Note that finding $\zeta^*_{o_i,g_i}$ and subsequently $C(\zeta^*_{o_i,g_i})$ implies the minimisation procedure in Equation \eqref{eq:back_plan}. This in turn poses a challenge in minimising \eqref{eq:back_minim}. However as shown in \cite{ratliff2006maximum} we can employ subgradient descent resulting in an update of the form,

\begin{equation}
	\nabla_{\mathbf{w}} =\frac{1}{D} \sum_{i=0}^D F(\zeta^i_{o_i,g_i}) - F(\zeta^*_{o_i,g_i})
\end{equation}

The inverse problem can therefore be seen as an iterative procedure, that first solves the planning problem in the inner loop, and based on that solution updates the weights in an appropriate manner in the outer loop. The weights at convergence represent the cost function that is used to plan the future behaviour of the agent. In \cite{ratliff2006maximum} the A$^*$ was used as a planning procedure for the inner loop assuming that the domain contained acyclic positive costs, in this paper we will make the same assumption and develop methods that use an RRT$^*$ as a base planner. 
...
\section{Methods}
	...
	\section{Future}
		...
		\subsection{Concerns}
		\begin{itemize}
			\item It could be very hard to incorporate the dynamic cases. I need to perfect the local planner which I dont have now. From what I have seen in related work, other researchers simply plan on a `snapshot' of a dynamic case and simply demonstrate that path. I general, evaluation for such work is a bit strange.
