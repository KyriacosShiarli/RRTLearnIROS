...
\end{equation}
where $Z_{o,g}$ is the set of all possible paths such that $s_0 = o, s_{l_\zeta} = g$.
...
While many formulations of the inverse problem exist, the general idea is to find a weight vector that assigns less cost to the example paths than all other possible paths with the same initial and goal configuration.  This can be formalised as an inequality constraint:
\begin{equation}
	C(\zeta^i_{o_i,g_i}) \leq \min_{\zeta_{o,g} \in Z_{o,g}} C(\zeta) \quad \forall i. \label{eq:const}
\end{equation}
...
\end{equation}
\sw{argmin minus a min? what is going on here?}
