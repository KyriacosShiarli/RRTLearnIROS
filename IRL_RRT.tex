...
When planning a path, a robot considers a space $\mathcal{C}$, of possible configurations $s \in \mathcal{C}$. These configurations are usually continuous, and often represent spacial quantities such as possition and orientation as well as their derivatives. A path planner is called to plan an obstacle free path $\zeta = (s_0,s_1,s_2$ $\ldots,s_{l_{\zeta}}) $ of length $l_{\zeta}$, from an initial configuration $s_0$ to a goal configuration  $s_g$. There could be however several paths to the goal state so path planners usualy employ a \emph{cost functional}, $C(\zeta)$, and choose the optimal path $\zeta^*$ which satisfies,
\begin{equation}
 	\zeta^* = \argmin_{\zeta \in Z_{0,g}} C(\zeta) \label{back_plan}
\end{equation}
where $Z_{0,g}$ is the set of all possible paths from $s_0$ to $s_g$, and may be infinite.
This cost functional is therefore similar to the one encountered in Optimal Control(OC) as well as Value which is used in Reinforcement Learning(RL). A fundamental difference however is that optimising $C(\zeta)$ in our case gives an open loop plan, instread for example a closed loop policy in the case of Reinforcement Learning. 

While the form of the cost functional may vary significantly depending on the application, it is very common to assign a cost function that assigns a cost to configuration pairs $c(s_i,s_j)$. The total cost $C(\zeta)$ of the path, is the sum of the individual costs along the path i.e.
\begin{equation}
...
The path planning problem above tackles the problem of finding an optimal (or near optimal) path to the goal given a cost function. In this paper we are concerned with the inverse problem, i.e. given example paths, find the cost function that will result in these paths being optimal (or near optimal). These example paths come in the form of a dataset $\mathcal{D} = (\tilde{\zeta_0},\tilde{\zeta_1}...\tilde{\zeta_D})$ where $\tilde{\zeta_i}$ denotes an observed path consisting in turn of various configuration sequences. Although the cost function that generated these observations is unknown, we assume access to a parametric form of the cost, namely,
\begin{equation}
	c(s_i,s_j) = \mathbf{w}^T \mathbf{f}(s_i,s_j)
\end{equation}
Where $\mathbf{w}$ is a K dimentional vector of unknown weights and $\mathbf{f}(s_i,s_j)$ is a feature vector of the same dimentionality. 
Using this notation and noting that $\mathbf{w}$ is not configuration dependant, we can express the total cost of the path in a parametric form,
