...
The first process is that of \emph{sampling}. A random point $s_{rand}$ is sampled from the configuration space. Next, the closest point $s_{closest}$, already in the existing vertex set $V$ is determined and a new point $s_{new}$ is created by \emph{steering} from  $s_{closest}$ to $s_{rand}$. This steering can be done in a way such that kinodynamic constraints are met. The final step in the sampling process is to determine the radius neighbours, $S_{near}$, of the point $s_{new}$.
...
%	\hspace{-5cm}
    ...
    \includegraphics[clip=true,width=1.28\textwidth]{images/cost_diff_train.png}
    \caption{Train}
 ...
 \hspace{5mm}
  \begin{subfigure}[b]{0.455\columnwidth}
    \includegraphics[clip=true,width=1.28\textwidth]{images/cost_diff_val.png}
    \caption{Test}
...
\end{figure}
	...
	\centering
	%\vspace{-3.3mm}
	    \includegraphics[width=0.22\textwidth]{images/path_compare.png}
	%\vspace{-4mm}
